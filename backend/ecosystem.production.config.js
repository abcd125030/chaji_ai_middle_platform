module.exports = {
    apps: [
        // Django 应用配置 - 生产环境
        {
            ...
            args: "backend.wsgi:application --bind 0.0.0.0:6066 --workers 20 --timeout 300 --graceful-timeout 60 --keep-alive 5 --worker-class sync",
            ...
            // PID 文件路径
            pid_file: "/www/wwwroot/repos/X/backend/logs/pids/backend.pid",
        ...
        ...Array.from({ length: 10 }, (_, i) => ({
            ...
            args: `-A backend worker --pool=gevent --concurrency=400 --loglevel=info --max-memory-per-child=2000000 --max-tasks-per-child=1000 --prefetch-multiplier=4 -n gevent_worker_${
                i + 1
        ...
        // 生产环境：2个worker，每个10并发
        ...Array.from({ length: 2 }, (_, i) => ({
            name: `celery_pdf_extractor_${i + 1}`,
            script: "/www/wwwroot/repos/X/backend/.venv/bin/celery",
            args: `-A backend.celery_pdf_extractor worker --pool=gevent --concurrency=10 --loglevel=info -n pdf_extractor_${i + 1}@%(h)s --pidfile=/www/wwwroot/repos/X/backend/logs/pids/celery_pdf_extractor_${i + 1}.pid`,
