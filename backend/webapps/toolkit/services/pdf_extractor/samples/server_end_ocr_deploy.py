...
def initialize_model(gpu_ids: str):
    """
    初始化模型，支持指定GPU卡
    :param gpu_ids: GPU卡ID，例如 "0" 或 "0,1"
    """
    global llm
    ...
    logger.info(f"Initializing DeepSeek-OCR model on {gpu_count} GPU(s): {gpu_ids}...")
    ...
    llm = LLM(
        ...
        tensor_parallel_size=gpu_count,  # 设置张量并行大小等于GPU数量
    )
    logger.info(f"Model initialized successfully on {gpu_count} GPU(s)!")
...
if __name__ == "__main__":
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="DeepSeek-OCR API Server")
    parser.add_argument(
        "--gpu",
        type=str,
        default="0",
        help="GPU卡ID，例如 '0' 表示使用0号卡，'0,1' 表示使用0和1两张卡 (默认: 0)"
    )
    parser.add_argument(
        "--port",
        type=int,
        default=9123,
        help="API服务端口 (默认: 9123)"
    )
    parser.add_argument(
        "--host",
        type=str,
